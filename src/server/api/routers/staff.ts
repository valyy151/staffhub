...
export const staffRouter = createTRPCRouter({
  ...
  createOrUpdate: protectedProcedure
    ...
    .mutation(
      async ({
        input: { id, firstName, lastName, email, address, phone },
        ctx,
      }) => {
        if (id) {
  ...
  getSickLeaves: protectedProcedure
    .input(z.object({ id: z.string() }))
    .query(async ({ input: { id }, ctx }) => {
      return await db.employee.findUnique({
        where: { id, userId: ctx.session.user.id },
  ...
  getVacations: protectedProcedure
    .input(z.object({ id: z.string() }))
    .query(async ({ input: { id }, ctx }) => {
      return await db.employee.findUnique({
        where: { id, userId: ctx.session.user.id },
    ...
    .query(async ({ input: { id, month }, ctx }) => {
      ...
      const schedule = newShifts.map((shift) => {
        const sickLeave = sickLeaves.find((sickLeave) => {
          return (
            shift.date * 1000 >= sickLeave.start &&
            shift.date * 1000 <= sickLeave.end
          )
        })

        const vacation = vacations.find((vacation) => {
          return (
            shift.date * 1000 >= vacation.start &&
            shift.date * 1000 <= vacation.end
          )
        })
