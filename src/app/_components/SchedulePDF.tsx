...
// export function SchedulePDF({ month, employee }: { month: string; employee: EmployeeProfile }) {
...
// 					<Text style={tw('px-4 py-[0.3572rem] flex flex-row border-b border-r w-1/4')}>{formatTotal(workDay.shift.start, workDay.shift.end)}</Text>
...
// 					{employee?.name} - {month} [{calculateTotalHours(employee?.workDays)} hours]
// 				</Text>
...
// 						<Text style={tw('px-4 py-[0.3572rem] border-b border-r w-1/4')}>{formatDate(workDay.date)}</Text>
// 						{renderDay(workDay)}
// 					</View>
// 				))}
// 			</Page>
