...
export const dashboardRouter = createTRPCRouter({
	find: protectedProcedure
		...
		.query(async ({ input: { page, month }, ctx }) => {
			const startOfWeek = new Date(month.getFullYear(), month.getMonth(), month.getDate() - month.getDay() + page * 7 + 1)
			...
			const [notes, shifts, workDays] = await Promise.all([notesPromise, shiftsPromise, workDaysPromise])
