...
export default function EditPreferences({
  ...
  employee: StaffPreferenceOutput
}) {
  ...
  const [hoursPerMonth, setHoursPerMonth] = useState(
    employee?.schedulePreference?.hoursPerMonth.toString() ?? "",
  )

  const [shiftModelIds, setShiftModelIds] = useState(
    employee?.schedulePreference?.shiftModels.map(
      (shiftModel) => shiftModel.id,
    ) ?? [],
  )
      ...
      {showEdit && (
        <AlertDialog open>
          <AlertDialogContent>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      e.target.checked
                        ...
                        : setShiftModelIds(
                            shiftModelIds.filter((id) => id !== shiftModel.id),
                          )
                    }}
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setShowEdit(false)}>
                Cancel
              </AlertDialogCancel>
              <AlertDialogAction
