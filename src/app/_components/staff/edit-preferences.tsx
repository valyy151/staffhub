...
export default function EditPreferences({ employee }: { employee: StaffPreferenceOutput }) {
	...
	const [shiftModelIds, setShiftModelIds] = useState(employee?.schedulePreference?.shiftModels.map((shiftModel) => shiftModel.id) ?? [])
				...
				<AlertDialog open>
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>Select the shift models and the amount of work hours per month.</AlertDialogDescription>
						</AlertDialogHeader>
							...
							{shiftModels?.map((shiftModel) => (
								<div className='flex'>
									<input
										...
										onChange={(e) => {
											e.target.checked ? setShiftModelIds([...shiftModelIds, shiftModel.id]) : setShiftModelIds(shiftModelIds.filter((id) => id !== shiftModel.id))
										}}
