...
export default function SelectEmployee({ links, name, employee, staff, setEmployee }: Props) {
		...
		<Popover
			...
			<PopoverTrigger asChild>
				<Button
					...
					{employee?.name || name ? employee?.name || name : 'Select an employee...'}
						...
						{staff
							...
							.map((employee) => (
								<CommandItem
									...
									<Check className={cn('mr-2 h-4 w-4', value === employee.name.toLowerCase() || employeeId === employee.id ? 'opacity-100' : 'opacity-0')} />
									{employee?.name}
								</CommandItem>
							))}
					</CommandGroup>
